...
class SystemCommands(BaseCommands):
    ...
    def performance_stats(self):
        ...
        try:
            ...
            if 'processing_times' in stats:
                ...
                typer.echo(f"  Average processing time: {format_duration(times.get('average', 0))}")
                typer.echo(f"  Min processing time: {format_duration(times.get('min', 0))}")
                typer.echo(f"  Max processing time: {format_duration(times.get('max', 0))}")

            # Memory usage
            if 'memory_usage' in stats:
                memory = stats['memory_usage']
                typer.echo(f"  Average memory usage: {format_file_size(memory.get('average', 0))}")
                typer.echo(f"  Peak memory usage: {format_file_size(memory.get('peak', 0))}")
            ...
            if 'trends' in stats:
                ...
                for period, data in trends.items():
                    typer.echo(f"  {period}: {data.get('operations', 0)} operations, "
    ...
    def sessions_list(self):
        ...
        try:
            ...
            typer.echo(f"{'ID':<8} {'Name':<20} {'Status':<12} {'Duration':<10} {'Created':<20}")
            typer.echo("-" * 80)
            ...
            for session in sessions:
                ...
                typer.echo(f"{session_id:<8} {name:<20} {status:<12} {duration:<10} {created:<20}")
            ...
            if success:
                ...
                while True:
                    ...
                    if total > 0:
                        ...
                        typer.echo(f"Progress: {completed}/{total} chunks ({percentage:.1f}%)")
    ...
    def sessions_delete(self, session_id: str):
        ...
        try:
            ...
            typer.confirm(f"Delete session '{session_name}' ({session_id})? This cannot be undone.", abort=True)
    ...
    def _display_benchmark_results(self, results: dict):
        ...
        if 'system_info' in results:
            ...
            typer.echo(f"  GPU Memory: {format_file_size(sys_info.get('gpu_memory', 0))}")
        ...
        if 'metrics' in results:
            ...
            typer.echo(f"  Model load time: {format_duration(metrics.get('model_load_time', 0))}")
            typer.echo(f"  Processing time: {format_duration(metrics.get('processing_time', 0))}")
            typer.echo(f"  Memory usage: {format_file_size(metrics.get('memory_usage', 0))}")
            typer.echo(f"  GPU utilization: {metrics.get('gpu_utilization', 0):.1f}%")
            typer.echo(f"  Audio processed: {format_duration(metrics.get('audio_duration', 0))}")
    ...
    def operations_list(self):
        ...
        try:
            ...
            typer.echo(f"{'ID':<8} {'Type':<15} {'Progress':<10} {'Status':<12} {'ETA':<10}")
            typer.echo("-" * 65)
            ...
            for op in operations:
                ...
                typer.echo(f"{op_id:<8} {op_type:<15} {progress:<10} {status:<12} {eta:<10}")
    ...
    def circuit_breaker_status(self):
        ...
        try:
            ...
            typer.echo(f"{'Service':<20} {'State':<10} {'Failures':<8} {'Success Rate':<12} {'Next Retry':<15}")
            typer.echo("-" * 75)
            ...
            for service_name, service_status in status.items():
                ...
                typer.echo(f"{service_name:<20} {state:<10} {failures:<8} {success_rate:<12} {next_retry:<15}")
    ...
    def circuit_breaker_reset(self, service: str | None = None):
        ...
        try:
            if service:
                ...
                if success:
                    display_progress(f"Circuit breaker reset for {service}", finished=True)
                else:
    ...
    def circuit_breaker_stats(self):
        ...
        try:
            ...
            for service_name, service_stats in stats.items():
                typer.echo(f"\n{service_name}:")
                typer.echo(f"  Total requests: {service_stats.get('total_requests', 0)}")
                typer.echo(f"  Successful requests: {service_stats.get('successful_requests', 0)}")
                typer.echo(f"  Failed requests: {service_stats.get('failed_requests', 0)}")
                typer.echo(f"  Circuit trips: {service_stats.get('circuit_trips', 0)}")
                typer.echo(f"  Average response time: {format_duration(service_stats.get('avg_response_time', 0))}")
